...
export function formatCliError(err, options = {}) {
  ...
  if (err instanceof Error) {
    if (options.verbose || options.stack || process.env.DEBUG) {
...
export function isPreCommitHookInstalled(options = {}) {
  ...
  try {
    ...
    const containsSelfReview = content.includes('self-review');
...
export function installPreCommitHook(options = {}) {
  ...
  if (fs.existsSync(hookPath)) {
    ...
    if (existing.includes('self-review')) {
      return {
        success: true,
    ...
    fs.writeFileSync(hookPath, existing + separator + addition, { mode: 0o755 });
    return {
      success: true,
      appended: true,
      hookPath,
  ...
  const content = `#!/bin/sh\n# Auto-generated by gem-pr-review pre-commit hook installer\n${command}\n`;
...
export function uninstallPreCommitHook(options = {}) {
  ...
  try {
    const content = fs.readFileSync(hookPath, 'utf8');
    if (!content.includes('self-review')) {
      return { success: true, removed: false, hookPath };
    }

    // If file only contains our auto-generated hook, delete file
    const lines = content.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
    const nonCommentLines = lines.filter((l) => !l.startsWith('#'));

    if (
      nonCommentLines.length <= 1 &&
      nonCommentLines.every((l) => l.includes('self-review'))
    ) {
      fs.unlinkSync(hookPath);
      return { success: true, removed: true, hookPath };
    }

    // If file contains other commands, strip self-review lines
    const cleaned = content
      .split(/\r?\n/)
      .filter((l) => !l.includes('self-review') && !l.includes('gem-pr-review self-review pre-commit hook'))
      .join('\n');
