...
- [x] Lead the documentation with the plugin identity: plugin-first README,
  ...
  install verified).
- [x] Land PR [#51](https://github.com/xpepper/pr-review-gemini/pull/51) (`docs/plugin-first-docs`)
  against `main` after addressing its dogfooding review by this tool itself
  (squash-merged as `d57c701` on 2026-09-14 after two review rounds).
- [x] Apply the residual plugin docs polish from the 2026-09-13 final review:
...
- [x] **Increment 22 / PR #40: Safe Verbose Review Diagnostics & Execution Telemetry**
  ...
  - [x] Integrated telemetry into `runReview` (`src/reviewer.js`) and `runSelfReview` (`src/self-review.js`), attaching diagnostic report on `--verbose`.
  - [x] Added `--verbose` (`-V`) and `--json` flag support to `scripts/dogfood-review.mjs`, `scripts/dogfood-pr.mjs`, `scripts/self-review.mjs`, and `src/cli.js`.
  - [x] Propagated `--verbose` in `/gem-review` comment command dispatcher and GitHub Action (`src/ci.js`, `action.yml`, `scripts/ci-action.mjs`).
  - [x] Added MCP tools `gem_pr_review_diagnostics` and `pr_review_diagnostics` in `server/index.js`.
  - [x] Added 36 unit tests across `tests/diagnostics.test.mjs`, `tests/ci.test.mjs`, `tests/dogfood.test.mjs`, `tests/mcp-server.test.mjs`, `tests/reviewer.test.mjs`, `tests/self-review.test.mjs`, and `tests/skills.test.mjs` (838 total tests passing across 158 suites).
  - [x] Dogfood-reviewed on PR #40 with 0 blocking findings and merged into `main` (commit `921e211`).
...
- [x] **Increment 19 / PR #33: Repository Review Guidelines & Domain Invariants (`.github/gem-pr-review.md`)**
  ...
  - [x] Hardened cache invalidation against concurrent SHA destruction, unified common CLI option parsing (`src/cli.js`), and preserved guidelines authenticity in MCP reviewer.
  - [x] Completed dogfood review on PR #33 with 0 blocking findings and squash-merged to `main`.
...
- [x] **Increment 17 / Issue #29: Centralize CLI Entrypoint Infrastructure and Eliminate Sibling Boilerplate Duplication**
  ...
  - [x] Added unit and integration tests in `tests/cli.test.mjs` and `tests/skills.test.mjs` (541 total passing across 109 suites with 0 failures).
  - [x] Dogfood review verified on GitHub PR #30 and merged to `main` (commit `d92295c`).
